...
let
  ...
  commons = {
    ...
    domains = {
      ...
      keycloak = "keycloak.koguma.iscute.ovh";
    };
  };
...

{
  ...
  containers.cmemu2 = baseContainer "cmemu" {
    config = { config, ... }: baseContainerConfig { name = "cmemu"; dns = true; } {
      users.users.cmemu = {
        createHome = true;
        home = "/home/cmemu";
      };
  ...
  containers.influxdb = baseContainer "influxdb" {
    forwardPorts = [
      { containerPort = 8086; hostPort = 8086; protocol = "tcp"; }
    ];
    config = { config, lib, ... }: baseContainerConfig { name = "influxdb"; dns = true; tcp = [8086]; } {
      services.influxdb = {
        enable = true;
  ...
  containers.keycloak = baseContainer "keycloak" {
    config = { config, ... }: baseContainerConfig { name = "keycloak"; dns = true; tcp = [80]; } {
      services.keycloak = {
        enable = true;
        database = {
          createLocally = false;
          ...
          type = "postgresql";
          useSSL = false;
          passwordFile = pkgs.writeText "pw" "";
        };
        frontendUrl = ''https://${commons.domains.keycloak}/auth'';
      };
    };
  };
    ...
    config = { config, ... }: baseContainerConfig { name = "nginx"; dns = true; tcp = [80 443]; } {
      ...
      services.nginx = {
	    ...
	    locations."/aoba/" = {
          ...
          # "${commons.domains.keycloak}" = {
          #   enableACME = true;
          #   forceSSL = true;
          #   locations."/auth" = {
          #     proxyPass = "http://${commons.ips.keycloak}:80";
          #   };
          # };
  ...
  containers.telegraf = baseContainer "telegraf" {
    config = { config, ... }: baseContainerConfig { name = "telegraf"; } {
      services.telegraf = {
        enable = true;
